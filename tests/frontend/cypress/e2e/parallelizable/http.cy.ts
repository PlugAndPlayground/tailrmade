...
describe('http', () => {
  ...
  it('see that the default request address results in an array of length larger than 10', () => {
    cy.wait(6000);
    doWithTestController((testController) => {
      const output = testController.getNodeOutputValue('HTTPNode', 'Content');
      expect(output.data.length).to.be.greaterThan(10);
    });
  });

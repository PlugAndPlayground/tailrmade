...
describe('break', () => {
  ...
  it('arrange them', () => {
    doWithTestController(async (testController) => {
      await testController.moveNodeByID('Break', 200, 0);
      await testController.moveNodeByID('Break2', 200, -200);
      await testController.moveNodeByID('Constant2', 0, -200);
    });
  });
  ...
  it('try a massive break node input, see that it results in 100 output sockets', () => {
    doWithTestController(async (testController) => {
      ...
      testController.setNodeInputValue('Constant2', 'In', {
        first: { second: 'hello' },
      });
  ...
  it('check the arrow json value is still cool and connected', () => {
    doWithTestController((testController) => {
      expect(testController.getNodeOutputValue('Break2', 'first→second')).to.eq(
        'hello',
      );

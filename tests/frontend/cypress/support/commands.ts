...
declare namespace Cypress {
  interface Chainable {
    ...
    failOnText(str: string): Chainable;
  }
}

Cypress.Commands.add('failOnText', (errorText: string) => {
  cy.get('body', { log: false }).then(($body) => {
    expect($body.text(), `Found error text: ${errorText}`).to.not.include(
      errorText,
    );
  });

  // Set up an observer to keep checking
  cy.document().then((doc) => {
    const observer = new MutationObserver(() => {
      const bodyText = doc.body.textContent || '';
      if (bodyText.includes(errorText)) {
        throw new Error(`Found error text: ${errorText}`);
      }
    });

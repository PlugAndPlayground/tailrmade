...
describe('theme store', () => {
  it('puts the saved layer under the runtime layer', () => {
    const layers = buildThemeLayers(
      { presetId: 'cloud', override: { radius: 1 } },
      { tokens: { radius: 20 } },
    );
  ...
  it('omits the runtime layer entirely when nothing pushed one', () => {
    expect(buildThemeLayers({ presetId: 'cloud' })).toHaveLength(1);
  });

  it('resolves outside React for the imperative paths', () => {
    setThemeDocument({ presetId: 'newsprint', mode: 'light' });
    expect(resolveAppThemeNow(true).presetId).toBe('newsprint');

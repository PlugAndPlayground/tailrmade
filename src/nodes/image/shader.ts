import * as PIXI from 'pixi.js';
import PPGraph from '../../classes/GraphClass';
import UpdateBehaviourClass from '../../classes/UpdateBehaviourClass';
import { TRgba } from '../../utils/color';
import { NODE_TYPE_COLOR, SOCKET_TYPE } from '../../utils/constants';
import Socket from '../../classes/SocketClass';
import { CodeType } from '../datatypes/codeType';
import { inputHeightName, inputWidthName } from '../draw/draw';
import { NumberType } from '../datatypes/numberType';
import { DRAW_Base, outputPixiName } from '../draw/abstract';
import { JSONType } from '../datatypes/jsonType';
import { ImageResourceMapType } from '../datatypes/imageResourceMapType';
import { NodeExecutionError } from '../../classes/ErrorClass';

const defaultVertex = `
in vec2 aPosition;
in vec2 aUV;
out vec2 UV;

uniform mat3 uProjectionMatrix;
uniform mat3 uWorldTransformMatrix;

uniform mat3 uTransformMatrix;


void main() {
    mat3 mvp = uProjectionMatrix * uWorldTransformMatrix * uTransformMatrix;
    gl_Position = vec4((mvp * vec3(aPosition, 1.0)).xy, 0.0, 1.0);

    UV = aUV;
}
`;

const defaultFragment = `in vec2 UV;
// these come in by default
uniform float time;
uniform float width;
uniform float height;

void main() {
    gl_FragColor = vec4(0.5,UV*vec2(1.,1.)*sin(time),1.0);
}
`;

const imageFragment = `in vec2 UV;
uniform sampler2D Image;
// these come in by default
uniform float time;
uniform float width;
uniform float height;

void main() {
  gl_FragColor = texture2D(Image,UV);
}
`;

const vertexShaderInputName = 'WebGL Vertex';
const fragmentShaderInputName = 'WebGL Fragment';
const inputUniformName = 'Uniforms';

const imageInputDataName = 'Images';
const renderTextureDataName = 'Texture';

const defaultImage =
  'data:image/jpeg;base64,';

export class Shader extends DRAW_Base {
  shader: PIXI.Shader;
  renderTexture: PIXI.RenderTexture;

  prevImageNames: string[] = [];
  prevGLVertex: string;
  prevGLFragment: string;

  protected getInitialVertex(): string {
    return defaultVertex;
  }
  protected getInitialFragment(): string {
    return defaultFragment;
  }

  protected getDefaultUniforms(): Record<string, any> {
    return {
      exampleUniformFloat: { value: 1.0, type: 'f32' },
      exampleUniformVector: { value: [640, 360, 1], type: 'vec3<f32>' },
    };
  }

  protected getDefaultImages(): Record<string, string> {
    return {};
  }

  getCanAddInput(): boolean {
    return true;
  }

  public getDescription(): string {
    return 'Draws a shader';
  }

  public getName(): string {
    return 'Draw shader';
  }

  protected shouldDraw(): boolean {
    return (
      !this.getOutputSocketByName(outputPixiName).hasLink() ||
      this.getOutputSocketByName(renderTextureDataName).hasLink()
    );
  }

  protected drawImmediately() {
    return this.getOutputSocketByName(renderTextureDataName).hasLink();
  }

  protected getDefaultIO(): Socket[] {
    return [
      new Socket(
        SOCKET_TYPE.IN,
        inputUniformName,
        new JSONType(),
        this.getDefaultUniforms(),
      ),
      new Socket(
        SOCKET_TYPE.IN,
        imageInputDataName,
        new ImageResourceMapType(),
        this.getDefaultImages(),
      ),
      new Socket(
        SOCKET_TYPE.IN,
        vertexShaderInputName,
        new CodeType(),
        this.getInitialVertex(),
        false,
      ),
      new Socket(
        SOCKET_TYPE.IN,
        fragmentShaderInputName,
        new CodeType(),
        this.getInitialFragment(),
      ),
      new Socket(
        SOCKET_TYPE.OUT,
        renderTextureDataName,
        new ImageResourceMapType(),
      ),
      new Socket(
        SOCKET_TYPE.IN,
        inputWidthName,
        new NumberType(false, 1, 1000),
        200,
      ),
      new Socket(
        SOCKET_TYPE.IN,
        inputHeightName,
        new NumberType(false, 1, 1000),
        200,
      ),
    ].concat(super.getDefaultIO());
  }

  getColor(): TRgba {
    return TRgba.fromString(NODE_TYPE_COLOR.SHADER);
  }

  public async drawOnContainer(
    input: any,
    container: PIXI.Container,
  ): Promise<void> {
    const currTime = (Date.now() % 1000) / 1000;

    const uniforms: Record<string, any> = JSON.parse(
      JSON.stringify(input[inputUniformName]),
    );
    const width = input[inputWidthName];
    const height = input[inputHeightName];
    if (width === 0 || height === 0) {
      return;
    }

    uniforms['time'] = { value: currTime, type: 'f32' };
    uniforms['width'] = { value: width, type: 'f32' };
    uniforms['height'] = { value: height, type: 'f32' };

    if (this.renderTexture === undefined || this.renderTexture.destroyed) {
      this.renderTexture = PIXI.RenderTexture.create({
        width: width,
        height: height,
      });
    }
    if (
      this.renderTexture !== undefined &&
      (this.renderTexture.width !== width ||
        this.renderTexture.height !== height)
    ) {
      this.renderTexture.resize(width, height);
    }

    // recreat everytime if there are images in here TODO optimize
    let hasChanged = false;
    if (
      input[vertexShaderInputName] !== this.prevGLVertex ||
      input[fragmentShaderInputName] !== this.prevGLFragment ||
      Object.keys(input[imageInputDataName]).length ||
      this.prevImageNames.length
    ) {
      hasChanged = true;
      const resources: any = {};
      // add the image resources
      try {
        for (const key of Object.keys(input[imageInputDataName])) {
          const curr = input[imageInputDataName][key];
          let resource: PIXI.TextureSource<any> | undefined = undefined;
          if (curr instanceof PIXI.RenderTexture) {
            resource = curr.source as PIXI.TextureSource<any>;
          } else if (curr instanceof PIXI.Texture) {
            resource = curr.source as PIXI.TextureSource<any>;
          } else if (typeof curr === 'string') {
            const texture = await PIXI.Assets.load<PIXI.Texture>(curr);
            resource = texture.source as PIXI.TextureSource<any>;
          } else {
            resource = PIXI.Texture.from(curr).source;
          }
          resources[key] = resource;
        }
      } catch (e) {
        console.warn('Failed to add images to shader', e);
      }
      this.prevImageNames
        .filter((name) => input[imageInputDataName][name] == undefined)
        .forEach((removed) => {
          resources[removed] = undefined;
        });
      this.prevImageNames = Object.keys(input[imageInputDataName]) || [];

      resources.shaderUniforms = uniforms;
      this.prevGLVertex = input[vertexShaderInputName];
      this.prevGLFragment = input[fragmentShaderInputName];
      this.shader = PIXI.Shader.from({
        gl: {
          vertex: this.prevGLVertex,
          fragment: this.prevGLFragment,
        },
        resources: resources,
      });
    } else if (this.shader) {
      // Update existing shader uniforms through the named group
      Object.entries(uniforms).forEach(([key, value]) => {
        this.shader.resources.shaderUniforms.uniforms[key] = value.value;
      });
    }

    const geometry = new PIXI.Geometry({
      attributes: {
        aPosition: [
          0,
          input[inputHeightName], // x, y
          input[inputWidthName],
          input[inputHeightName], // x, y
          input[inputWidthName],
          0,
          0,
          0,
        ],
        aUV: [0, 1, 1, 1, 1, 0, 0, 0],
      },
      indexBuffer: [0, 1, 2, 0, 2, 3],
    });

    const graphics = new PIXI.Mesh({
      geometry,
      shader: this.shader,
    });
    const newContainer = new PIXI.Container();
    newContainer.addChild(graphics);

    // Capture any console errors from PIXI.js during render if it has changed
    const capturedErrors: any[] = [];
    const originalConsoleError = console.error;
    if (hasChanged) {
      console.error = (...args: any[]) => {
        capturedErrors.push(args);
        originalConsoleError.apply(console, args);
      };
    }

    try {
      PPGraph.currentGraph.app.renderer.render({
        container: newContainer,
        target: this.renderTexture,
      });

      // Log any captured errors with context
      if (capturedErrors.length > 0) {
        console.warn('SHADER RENDER ERRORS CAPTURED:', capturedErrors);
        this.setStatus(new NodeExecutionError(capturedErrors.join('\n')));
      }
    } finally {
      // Restore original console.error
      if (hasChanged) {
        console.error = originalConsoleError;
      }
    }

    container.addChild(newContainer);

    this.setOutputData(renderTextureDataName, { Image: this.renderTexture });
  }

  onNodeRemoved = () => {
    if (this.renderTexture !== undefined && !this.renderTexture.destroyed) {
      this.renderTexture.destroy();
    }
  };

  public getUpdateBehaviour(): UpdateBehaviourClass {
    return new UpdateBehaviourClass(true, true, false, 16, this);
  }
}

export class ImageShader extends Shader {
  public getName(): string {
    return 'Draw image shader';
  }

  public getDescription(): string {
    return 'Draws an image rendered in a shader';
  }

  public getTags(): string[] {
    return ['Media'].concat(super.getTags());
  }

  protected getInitialFragment(): string {
    return imageFragment;
  }

  protected getDefaultImages(): Record<string, string> {
    return { Image: defaultImage };
  }
}

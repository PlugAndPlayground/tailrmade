...
export class PixotopeGatewayGet extends HTTPNode {
  ...
  protected async onExecute(
    ...
    const outputValue = response[0]?.Message?.Value;
    if (outputValue == undefined) {
      outputObject[outputContentName] = {};
    } else {
      outputObject[outputContentName] = outputValue;
    }
  }
  ...
  protected getDefaultIO(): Socket[] {
    return [
      ...
      new Socket(
        ...
        nameName,
        new StringType(),
        'State.ThirdParty.TailrmadeSettable',
      ),
  ...
  protected getDefaultIO(): Socket[] {
    return [
      ...
      new Socket(SOCKET_TYPE.IN, paramsName, new JSONType(), {
        Fingerprint: 'Tailrmade',
        Name: 'TailrmadeCamera',
      }),
      new Socket(SOCKET_TYPE.IN, timeoutName, new NumberType(), 1000),
      new Socket(SOCKET_TYPE.OUT, outputContentName, new JSONType(), {}),
    ];
  }
  protected async onExecute(
    ...
    const address = inputObject[urlInputName];
    const timeout = inputObject[timeoutName];
    ...
    const outputValue = res[0]?.Message?.Result;
    if (outputValue == undefined) {
      outputObject[outputContentName] = {};
    } else {
      outputObject[outputContentName] = outputValue;
